...
@author: Ash Booth adapted for BSE by Mihai Nemes
...
class Trader_AA(Trader):
    ...
    eqlbm = None
    theta = -1.0 * (5.0 * random.random())
    smithsAlpha = None
    lastTrades = []
    smithsAlphaMin = None
    smithsAlphaMax = None
        
    aggressiveness_buy = -1.0 * (0.3 * random.random())
    aggressiveness_sell = -1.0 * (0.3 * random.random())
    target_buy = None
    target_sell = None
    ...
    def updateTarget(self):
        ...
        # For buying
        if self.limit != None and self.eqlbm != None:
    ...
    def calcRshout(self, target, buying):
        ...
        if target != None and self.limit != None and self.eqlbm != None:
            if buying:
    ...
    def updateTheta(self):
        if self.smithsAlpha != None and self.smithsAlphaMin != None and self.smithsAlphaMax != None and self.smithsAlphaMin:
    ...
    def getorder(self, time, countdown, lob):
        if len(self.orders) < 1:
            ...
            if self.job == 'Bid':            
                ...
                quoteprice = None
                if self.spin_up_time > 0 and self.prev_best_ask_p != None and self.prev_best_bid_p != None:
                ...
                elif self.prev_best_bid_p != None and self.prev_best_bid_p != None and self.target_buy != None:
                    quoteprice = self.prev_best_bid_p + (self.target_buy - self.prev_best_bid_p) / self.eta
            else:
                # currently a seller (working a sell order)
                quoteprice = None
                if self.spin_up_time > 0 and self.prev_best_bid_p != None and self.prev_best_ask_p != None:
                ...
                elif self.prev_best_bid_p != None and self.prev_best_ask_p != None and self.target_sell != None:
    ...
    def respond(self, time, lob, trade, verbose):
        ...
        if lob_best_bid_p != None:
            ...
            if self.prev_best_bid_p < lob_best_bid_p :
                ...
                bid_improved = True
            elif trade != None and ((self.prev_best_bid_p > lob_best_bid_p) or ((self.prev_best_bid_p == lob_best_bid_p))):
        ...
        if lob_best_ask_p != None:
            ...
            if self.prev_best_ask_p > lob_best_ask_p :
                ...
                ask_improved = True
            elif trade != None and ((self.prev_best_ask_p < lob_best_ask_p) or ((self.prev_best_ask_p == lob_best_ask_p))):
        ...
        if trade != None:
            ...
            elif bid_improved and (self.target_buy <= price): 
            ...
            if deal:
                ...
                self.updateSmithsAlpha(price)
                self.updateTheta() 
                if self.target_sell <= price:  self.aggressiveness_sell = self.updateAgg(False, False, price)
                else: self.aggressiveness_sell = self.updateAgg(True, False, price)
            elif ask_improved and (self.target_sell >= price): 
